default_entities = [
    ...
    },
    {
        ...
        'events':       {
            ...
            'timestep': 'var player=$entity_nearest_player;var self=$this;var playerNearby=(__distance(player.location,self.location)<6);if(playerNearby){self.meta.roamProgress=0;self.meta.roamPath=[];var newLocation=__moveTo({\'location\':self.location,\'target\':player.location});if(newLocation[0]!==undefined){self.location.x=newLocation[0].x;self.location.y=newLocation[0].y}}else{self.meta.roamProgress=self.meta.roamProgress||0;self.meta.roamPath=self.meta.roamPath||[];if((self.meta.roamProgress+1)>(self.meta.roamPath.length-1)){self.meta.roamProgress=0;var isObstacle=true;var targetLocation;while(isObstacle){targetLocation={\'x\':self.location.x-__rand(-10,10),\'y\':self.location.y-__rand(-10,10)};if(!__isObstacle(targetLocation)){if(__distance(self.location,targetLocation)>2){isObstacle=false}}}self.meta.targetLocation=targetLocation;self.meta.roamPath=__moveTo({\'location\':self.location,\'target\':targetLocation})}var newLocation=self.meta.roamPath[self.meta.roamProgress];self.meta.roamProgress=self.meta.roamProgress+1;if(newLocation!==undefined){self.location.x=newLocation.x;self.location.y=newLocation.y}}'
        },
        'meta':        {}
    },
    {
