...
@app.route('/currency/transfer/', methods=['GET', 'POST'])
@login_required
def currency_tansfer():
            ...
            try:
                ...
                if recipient_key:
                    try:
                        ...
                        Notification.send(recipient_key, '%s sent you %d SS and %d CC' % (g.user.display_name, ss, cc), '/currency/transfer/')

                        return redirect(url_for('currency_tansfer'))
                else:
                    flash('The user you are trying to send money to does not exist!', 'error')

    return render_template("currency/transfer.html", recipient=recipient, ss=ss, cc=cc)
